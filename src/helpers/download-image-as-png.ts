...
export const downloadImageAsPng =async(url:string, fullPath:boolean = false)=>{
  ...
  if(!response.ok){
      throw new InternalServerErrorException('No es posible descargar la imagen');
  }

  //Sirve para localizar y crear la ruta de descarga
  const folderPath = path.resolve('./', './generated/images/'); 
  fs.mkdirSync( folderPath, {recursive:true} );

  //Creo un uuid para el archivo utilizando la fecha
